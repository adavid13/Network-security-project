...
def active_ips():
	...
	par1 = 0 #flag for the index to cut based on length of output (the number of spaces changes if the result has 10 connections vs 1000)
	par2 = 0 #flag 2
	if(output): #checks if traffic was captured
		...
		for i in range(len(spltOut)): #iterate through the lines of the output
			...
			if(len(line)>0): #remove empty lines
				...
				#determine at which index the line will be split based on the length of the output
				if(len(spltLine)==8):
					par1=6
					par2=7
				if(len(spltLine)==7):
